...
function takeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("~r~You need a pistol with ammo to take a hostage at gunpoint!~r~")
	end
	...
	if not holdingHostageInProgress and canTakeHostage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("~r~No one nearby to take as hostage nigga!~r~")
		end 
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"~g~Press [G] to release~g~, ~r~[H] to kill~r~")
